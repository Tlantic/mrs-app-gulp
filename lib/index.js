...
/* PROJECT INFO */
_defaults = util.retrieveProjectInfo($path.normalize(process.cwd(), 'package.json'));
...
if (!_defaults.paths) {
    ...
    /* BUILD SETTINGS */
    _defaults.buildSettings = util.retrieveBuildSettings($path.normalize(paths.config.$brand.root, 'build.json'));
}
...
module.exports = {
    ...
    init: function (gulp, options) {

        options = util.extend(options || {}, util.retrieveProjectInfo($path.normalize(process.cwd(), 'package.json')));
        ...
        return mrsGulp(gulp).init(options);
    }
};